        ...
        <div class="pt-5">
            <div class="panel pb-0">
                ...
                <Datatable :rows="hostings" :columns="columns" :totalRows="hostings.data?.length" @change="apply_filter"
                    ...
                    <template v-if="$can('edit_hostings') || $can('delete_hostings') || $can('restore_hostings')"
                        #actions="data">
                        ...
                        <div v-else class="flex gap-2">
                            ...
                            <div v-if="$can('restore_hostings')" class="text-center">
...
const save = () => {
    ...
    if (form?.id) {
        form.post(route('control.pages.hostings.update', form), {
            onSuccess: () => {
                toggleModal();
...
const callForceDelete = (row) => {
    Swal.fire({
        ...
        if (result.value) {
            form.delete(route('control.pages.hostings.force_delete', row.id), {
...
const callRestore = (row) => {
    Swal.fire({
        ...
        if (result.value) {
            form.post(route('control.pages.hostings.restore', row.id), {
