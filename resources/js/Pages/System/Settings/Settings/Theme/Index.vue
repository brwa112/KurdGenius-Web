    ...
    <div class="mx-auto">
        ...
        <div class="pt-4">
            <div class="panel pb-0">
                <!-- Datatable -->
                <Datatable :rows="themes" :columns="columns" :totalRows="themes.data?.length" @change="apply_filter"
                    ...
                    <template v-if="$can('edit_themes') || $can('delete_themes')" #actions="data">
                        <div class="flex gap-2">
                            <div v-if="$can('edit_themes')" class="text-center">
                                <button type="button" v-tippy @click="toggleModal(data.value)">
                                    <Svg name="pencil" class="size-5"></Svg>
                                </button>
                                <tippy>{{ $t('common.edit') }}</tippy>
                            </div>
                            <div v-if="$can('delete_themes')" class="text-center">
...
const save = () => {
    ...
    if (form.id) {
        ...
        form.put(route('control.system.settings.theme.update', form.id), {
            onSuccess: () => {
                toggleModal();
    ...
    // Create new record
    form.post(route('control.system.settings.theme.store'), {
        onSuccess: () => {
            toggleModal();
...
const columns =
    ref([
        ...
        {
            field: 'name',
            title: wTrans('system.name')
        },
        {
            field: 'slug',
            title: wTrans('system.slug')
        },
...
const callDelete = (id) => {
    Swal.fire({
        ...
        if (result.value) {
            form.delete(route('control.system.settings.theme.destroy', id), {
                onSuccess: () => {
