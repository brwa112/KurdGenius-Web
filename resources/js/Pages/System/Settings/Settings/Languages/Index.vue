    ...
    <div class="mx-auto">
                                        ...
                                        <div class="p-5">
                                            <form @submit.prevent="save()" v-shortkey="['ctrl', 's']" @shortkey="save"
                                                ...
                                                <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 text-sm">
                                                    ...
                                                    <div class="col-span-full">
                                                        <label class="required" for="direction">{{
                                                            $t("system.direction") }}</label>
                                                        <MultiSelect id="direction" v-model="form.direction" :list="direction"
                                                    ...
                                                    <div class="col-span-full">
                                                        ...
                                                        <input  type="file" id="file" @change="handleFile"
                                                            accept=".xlsx,.xls" />
                                                        <div class="mt-1 text-danger" v-if="form.errors.file"
                                                            >
                                                            {{ form.errors.file }}
                                                        </div>
                                                    </div>
        ...
        <div class="pt-4">
            <div class="panel pb-0">
                <!-- Datatable -->
                <Datatable :rows="language" :columns="columns" :totalRows="language.data?.length" @change="apply_filter"
                    ...
                    <template v-if="$can('edit_languages') || $can('delete_languages')" #actions="data">
                        <div class="flex gap-2">
                            <div v-if="$can('edit_languages')" class="text-center">
                                <button type="button" v-tippy @click="toggleModal(data.value)">
                                    <Svg name="pencil" class="size-5"></Svg>
                                </button>
                                <tippy>{{ $t('common.edit') }}</tippy>
                            </div>
                            <div v-if="$can('delete_languages')" class="text-center">
...
const toggleModal = (row) => {
    if (row) {
        console.log(row);
        form.id = row.id;
        form.direction = direction.find(dir => dir.value === row.direction) || '';
        form.name = row.name;
        form.slug = row.slug;
    }
...
const save = () => {
    ...
    if (form.id) {
        // Option 1: Use POST with _method spoofing for file uploads
        form.transform((data) => ({
            ...data,
            _method: 'PUT'
        })).post(route('control.system.settings.languages.update', form.id), {
            forceFormData: true,
            onSuccess: () => {
                toggleModal();
    ...
    form.post(route('control.system.settings.languages.store'), {
        forceFormData: true,
        onSuccess: () => {
            toggleModal();
...
const callDelete = (data) => {
    console.log(data);
    if (data.can_be_deleted === false) {
        Swal.fire({
            icon: 'error',
            title: trans('common.cannot_delete'),
            text: trans('common.has_attached_language'),
        ...
        Swal.fire({
            ...
            if (result.value) {
                form.delete(route('control.system.settings.languages.destroy', data.id), {
                    onSuccess: () => {
