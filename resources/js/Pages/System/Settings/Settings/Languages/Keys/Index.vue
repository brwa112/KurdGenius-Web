    ...
    <div class="mx-auto">
                                                ...
                                                <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 text-sm">
                                                    <div class="col-span-full">
                                                        <div class="col-span-full">
                                                            ...
                                                            <VInput id="key" v-model="form.key" type="text"
                                                                :placeholder="$t('system.key')"
        ...
        <div class="pt-4">
            <div class="panel pb-0">
                <!-- Datatable -->
                <Datatable :rows="keys" :columns="columns" :totalRows="keys.data?.length" @change="apply_filter"
                    ...
                    <template v-if="$can('edit_keys') || $can('delete_keys')" #actions="data">
                        <div class="flex gap-2">
                            <div v-if="$can('edit_keys')" class="text-center">
                                <button type="button" v-tippy @click="toggleModal(data.value)">
                                    <Svg name="pencil" class="size-5"></Svg>
                                </button>
                                <tippy>{{ $t('common.edit') }}</tippy>
                            </div>
                            <div v-if="$can('delete_keys')" class="text-center">
...
// Computed property to handle the date range display in the CustomDatePicker
...
const save = () => {
    ...
    if (form.id) {
        ...
        form.put(route('control.system.settings.keys.update', form.id), {
            onSuccess: () => {
                toggleModal();
    ...
    // Create new record
    form.post(route('control.system.settings.keys.store'), {
        onSuccess: () => {
            toggleModal();
...
const callDelete = (id) => {
    Swal.fire({
        ...
        if (result.value) {
            form.delete(route('control.system.settings.keys.destroy', id), {
                onSuccess: () => {
