<template>
    ...
    <Head>
        <title>{{ $t('system.group_permissions') }}</title>
    </Head>
    <div class="mx-auto">
        <div class="w-full flex flex-wrap items-center justify-between gap-x-5 gap-y-2.5 -mt-1">
            <ul class="flex flex-wrap  space-x-2 rtl:space-x-reverse">
                ...
                <li class="before:content-['/'] ltr:before:mr-2 rtl:before:ml-2">
                    <span>{{ $t('system.group_permissions') }}</span>
                </li>
            </ul>
            <!-- add new row -->
            <div class="block">
                                            ...
                                            <form @submit.prevent="save()" v-shortkey="['ctrl', 's']" @shortkey="save"
                                                ...
                                                <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 text-sm">
                                                    ...
                                                    <div class="col-span-full">
                                                        ...
                                                        <VInput id="description" v-model="form.description" type="text"
        ...
        <div class="pt-4">
            <div class="panel pb-0">
                <!-- Datatable -->
                <Datatable :rows="group_permissions" :columns="columns" :totalRows="group_permissions.data?.length" @change="apply_filter"
                    ...
                    <template v-if="$can('edit_group_permissions') || $can('delete_group_permissions')" #actions="data">
                        <div class="flex gap-2">
                            <div v-if="$can('edit_group_permissions')" class="text-center">
                                <button type="button" v-tippy @click="toggleModal(data.value)">
                                    <Svg name="pencil" class="size-5"></Svg>
                                </button>
                                <tippy>{{ $t('common.edit') }}</tippy>
                            </div>
                            <div v-if="$can('delete_group_permissions')" class="text-center">
                                <button type="button" v-tippy @click="callDelete(data)">
...
const save = () => {
    ...
    if (form.id) {
        ...
        form.put(route('control.system.group_permissions.update', form.id), {
            onSuccess: () => {
                toggleModal();
    ...
    // Create new record
    form.post(route('control.system.group_permissions.store'), {
        onSuccess: () => {
            toggleModal();
...
const callDelete = (data) => {
    if (data.value.group_permissions_count > 0) {
        Swal.fire({
            icon: "error",
            title: trans("common.action_not_allowed"),
            text: trans('common.cannot_delete_active_group_permissions'),
        });
    } else {
        Swal.fire({
            icon: 'warning',
            ...
            if (result.value) {
                form.delete(route('control.system.group_permissions.destroy', data.value.id), {
                    onSuccess: () => {
