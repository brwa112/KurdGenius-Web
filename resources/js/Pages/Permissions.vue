...
const submit = () => {
    if (editPermission.value) {
        form.put(route('control.permissions.update', editPermission.value.id), {
            onSuccess: () => resetForm()
        });
    } else {
        form.post(route('control.permissions.store'), {
            onSuccess: () => resetForm()
        });
    }
};
...
const destroy = (id) => {
    if (confirm("Are you sure?")) {
        form.delete(route('control.permissions.destroy', id));
    }
};
